...
u8 *Pia_PORTA(_6502_Context_t *pContext, u8 *pValue)
{
	...
	{
		if(pValue)
			pAtariData->cValuePortA = *pValue;

		return &pAtariData->cValuePortA;
	}
...
u8 *Pia_PORTB(_6502_Context_t *pContext, u8 *pValue)
{
	...
	{
		if(pValue)
			pAtariData->cValuePortB = *pValue;

		return &pAtariData->cValuePortB;
	}
	...
	if(pValue)
	{
		...
		if((SRAM[IO_PORTB] & 0x80) != (*pValue & 0x80))
		{
			...
			else
			{
				...
				memcpy(&RAM[0x5000], pAtariData->pSelfTestRom, 0x0800);
			}
		}
